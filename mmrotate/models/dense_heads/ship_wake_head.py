...
class ShipPointHead(nn.Module):
    ...
    def forward_single(self, x):
        ...
        dir_norm = torch.sqrt(
            direction_pred[:, :, 0] ** 2 + direction_pred[:, :, 1] ** 2 + 1e-6
        )
    ...
    def get_targets(
        ...
        return (
            ...
            torch.cat(all_label_weights, 0),
            torch.cat(all_center_targets, 0),
            torch.cat(all_direction_targets, 0),
            torch.cat(all_center_weights, 0),
            torch.cat(all_direction_weights, 0),
            pos_inds_list,
            neg_inds_list,
        )
    ...
    def loss(
        ...
        flatten_direction_preds = torch.cat(
            [
                pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 2)
                for pred in direction_preds
            ],
            dim=1,
        ).reshape(-1, 2)
        flatten_conf_preds = torch.cat(
            [pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 1) for pred in conf_preds],
            dim=1,
        ).reshape(-1, 1)
        ...
        if num_pos > 0:
            ...
            loss_center = self.loss_center(
                flatten_center_preds[pos_mask],
                center_targets[pos_mask],
                center_weights[pos_mask],
                avg_factor=num_pos,
            )
            loss_direction = self.loss_direction(
                flatten_direction_preds[pos_mask],
                direction_targets[pos_mask],
                direction_weights[pos_mask],
                avg_factor=num_pos,
            )
        else:
        ...
        return {
            ...
            "loss_conf": loss_conf,
            "num_pos": loss_center.new_tensor(
    ...
    def get_points_predictions(
        ...
        for img_id in range(num_imgs):
            ...
            pred_labels = torch.ones(
                len(pred_bboxes), dtype=torch.long, device=pred_bboxes.device
            )
        ...
        for ship_res, wake_res in zip(ship_results, wake_results):
            ...
            if isinstance(wake_res, tuple):
                if len(wake_res) == 3:
                    ...
                    wake_scores = torch.ones(
                        len(wake_bboxes), device=wake_bboxes.device
                    )
            else:
                # Single tensor format - use as bboxes
                wake_bboxes = wake_res
                wake_labels = torch.ones(
                    len(wake_bboxes), dtype=torch.long, device=wake_bboxes.device
                )
            ...
            target_device = (
                ship_bboxes.device
            )  # ship_bboxes is guaranteed to be on the right device
